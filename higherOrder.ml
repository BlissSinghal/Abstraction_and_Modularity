...
(******************************************************************************)
(* PROBLEM 4: ANONYMOUS & HIGHER-ORDER FUNCTIONS                              *)
(******************************************************************************)
...
(* Now, use `fold` to write a `concat` function that takes a list of lists and
   ...
   enclose the expression in parens to avoid confusing the parser.)
   Do not add `rec` to the declaration of `concat`. *)
...
let for_all (pred: 'a -> bool) (l: 'a list) : bool =
  fold (fun x acc -> acc && pred x) true l
...
let exists (pred: 'a -> bool) (l: 'a list) : bool =
  fold (fun x acc -> acc || pred x) false l 
