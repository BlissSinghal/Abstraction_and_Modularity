...
let texts () : (string * string set) list =
   (*just need to convert the list to set*)
   transform (fun (name, words) -> (name, set_of_list words)) texts_as_lists
...
let count_if (pred: 'a -> bool) (l: 'a list) : int =
  let filtered_l = filter pred l in
  list_length filtered_l
...
;; run_test "count_sat_words empty list" test

let test () : bool =
  count_sat_words (set_of_list ["ardor"]) = 1
 ;; run_test "count_sat_words singleton that returns true" test 

let test() : bool = 
  count_sat_words (set_of_list ["hi"; "dissent"; "thing"; "ardor"]) = 2
 ;; run_test "count_sat_words list with both sat and non sat words" test 
...
let count_length_gt7 (s: string set) : int =
  let s_list = list_of_set s in
  count_if (fun x -> String.length x > 7) s_list
